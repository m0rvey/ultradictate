...
grep -q 'm0rvey/ultradictate' "$readme_file"
grep -q '^REF="${ULTRADICTATE_REF:-\$SOURCE_COMMIT}"$' install.sh
grep -q '^EXPECTED_SOURCE_COMMIT="${ULTRADICTATE_SOURCE_COMMIT:-\$SOURCE_COMMIT}"$' install.sh
...
git diff --check
printf 'UltraDictate checks passed (v%s).\n' "$app_version"
