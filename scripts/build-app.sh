...
OUTPUT_APP="${1:-$ROOT_DIR/dist/UltraDictate.app}"
SIGN_IDENTITY="${SIGN_IDENTITY:--}"

say() {
    printf 'UltraDictate: %s\n' "$*"
}
...
BIN="$BIN_DIR/UltraDictate"
[[ -x "$BIN" ]] || fail "The Swift build did not produce $BIN"

STAGE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ultradictate-build.XXXXXX")"
trap 'rm -rf "$STAGE_DIR"' EXIT
STAGE_APP="$STAGE_DIR/UltraDictate.app"

mkdir -p "$STAGE_APP/Contents/MacOS" "$STAGE_APP/Contents/Resources"
cp "$BIN" "$STAGE_APP/Contents/MacOS/UltraDictate"
...
chmod 755 "$STAGE_APP/Contents/MacOS/UltraDictate"
