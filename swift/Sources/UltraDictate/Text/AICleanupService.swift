...
enum AICleanupService {
    ...
    static func clean(text: String, language: DictationLanguage) async throws -> String {
        ...
        let storedKey = AIKeyStore.read()?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard isLocal || !storedKey.isEmpty else {
            throw AICleanupError.noAPIKey
        }
        let authHeaderKey = storedKey.isEmpty ? "ollama" : storedKey

        var req = URLRequest(url: url)
        req.httpMethod = "POST"
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        req.setValue("Bearer \(authHeaderKey)", forHTTPHeaderField: "Authorization")
    ...
    static func probeModels(baseURL: String,
        ...
        let isLocal = isLocalEndpoint(base)
        guard isLocal || !apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return .failure(.noAPIKey)
        }
        let authHeaderKey = apiKey.isEmpty ? "ollama" : apiKey
        var req = URLRequest(url: url)
        req.setValue("Bearer \(authHeaderKey)", forHTTPHeaderField: "Authorization")
