...
REPOSITORY="${ULTRADICTATE_REPOSITORY:-m0rvey/ultradictate}"
RELEASE_VERSION="1.0.0"
RELEASE_SHA256="036cf2eaa8a882d525f017373c41a8011a523b204802dab4f1714479701aa502"
SOURCE_COMMIT="HEAD"
RELEASE_URL="${ULTRADICTATE_RELEASE_URL:-https://github.com/$REPOSITORY/releases/download/v$RELEASE_VERSION/UltraDictate-macOS-arm64.zip}"
EXPECTED_SHA256="${ULTRADICTATE_RELEASE_SHA256:-$RELEASE_SHA256}"
REF="${ULTRADICTATE_REF:-$SOURCE_COMMIT}"
EXPECTED_SOURCE_COMMIT="${ULTRADICTATE_SOURCE_COMMIT:-$SOURCE_COMMIT}"
APP_PATH="${ULTRADICTATE_APP_PATH:-/Applications/UltraDictate.app}"
BUILD_FROM_SOURCE="${ULTRADICTATE_BUILD_FROM_SOURCE:-0}"
NO_OPEN="${ULTRADICTATE_NO_OPEN:-0}"
AGENT_LABEL="com.local.ultradictate.agent"

say() {
    printf '\033[1;36mUltraDictate:\033[0m %s\n' "$*"
}

fail() {
    printf '\033[1;31mUltraDictate:\033[0m %s\n' "$*" >&2
    exit 1
}
...
verify_app() {
    local app="$1"
    local executable="$app/Contents/MacOS/UltraDictate"
    local bundle_id version minimum_system entitlements_file audio_input microphone

    [[ -x "$executable" ]] || fail "В архиве нет исполняемого файла UltraDictate."
    bundle_id="$(plutil -extract CFBundleIdentifier raw -o - "$app/Contents/Info.plist")"
    [[ "$bundle_id" == "com.m0rvey.ultradictate" ]] || fail "Неверный идентификатор приложения: $bundle_id"
...
download_release() {
    local work_dir="$1"
    local archive="$work_dir/UltraDictate-macOS-arm64.zip"
    local actual
    ...
    [[ -d "$work_dir/release/UltraDictate.app" ]] || fail "В релизе нет UltraDictate.app."
    ditto "$work_dir/release/UltraDictate.app" "$work_dir/UltraDictate.app"
}
...
build_from_source() {
    ...
    command -v swift >/dev/null 2>&1 || {
        say "Для сборки из исходников нужны инструменты Apple. Открываю их установку..."
    ...
    "$source_dir/scripts/build-app.sh" "$work_dir/UltraDictate.app"
}
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/ultradictate-install.XXXXXX")"
...
verify_app "$WORK_DIR/UltraDictate.app"
say "Устанавливаю приложение в $APP_PATH..."

if [[ "$APP_PATH" == "/Applications/UltraDictate.app" ]]; then
    /bin/launchctl bootout "gui/$UID/$AGENT_LABEL" >/dev/null 2>&1 || true
    /usr/bin/pkill -x UltraDictate >/dev/null 2>&1 || true
fi

INCOMING="$(dirname "$APP_PATH")/.UltraDictate.install.$$"
BACKUP="$(dirname "$APP_PATH")/.UltraDictate.previous.$$"
run_as_admin rm -rf "$INCOMING"
run_as_admin rm -rf "$BACKUP"
run_as_admin ditto "$WORK_DIR/UltraDictate.app" "$INCOMING"
verify_app "$INCOMING"
...
if [[ "$NO_OPEN" == "1" ]]; then
    ...
    say "Готово. Открываю UltraDictate..."
    open "$APP_PATH"
